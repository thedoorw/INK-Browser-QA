...
const evidence = path.join(root, 'tests', 'browser-evidence-v0.1');
const reportFile = path.join(evidence, 'browser-health-report-v0.1.json');
  ...
  try {
    ...
    for (let i = 0; i < 150; i++) {
      const result = await pageConnection.send('Runtime.evaluate', {
        expression: `JSON.stringify({href:location.href,ready:document.readyState,title:document.title,text:(document.body?.innerText||'').slice(0,160),stage:!!document.querySelector('#stage'),app:!!window.INK_STUDIO?.app,doc:!!window.INK_STUDIO?.app?.doc,docVersion:window.INK_STUDIO?.app?.doc?.appVersion||null,studioVersion:window.INK_STUDIO?.version||null,aiVersion:window.INK_AI?.version||null})`,
        returnByValue: true
      });
    ...
    if (state.href?.startsWith('chrome-error://') && /blocked|organization/i.test(state.text || '')) {
      ...
      check('user-facing title uses INK v0.1', /^INK v0\.1\b/.test(state.title || ''), state);
      check('document appVersion is 0.1', state.docVersion === '0.1', state);
