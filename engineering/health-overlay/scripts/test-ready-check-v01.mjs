...
const reportFile = path.join(root, 'tests', 'browser-evidence-v0.1', 'browser-health-report-v0.1.json');
...
const valid = report.version === '0.1' && report.status === 'PASS' && failedChecks.length === 0 && (report.pageExceptions || []).length === 0 && (report.criticalConsoleErrors || []).length === 0;
if (!valid) {
...
console.log(JSON.stringify({ status: 'PASS', version: report.version, stage: report.stage || null, reportFile, checks: report.checks.length }, null, 2));
