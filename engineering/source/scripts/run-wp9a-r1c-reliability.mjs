...
const serialized = fs.readFileSync(new URL('../FLR012_PREVIEW_ONLY_Painted_Retention.ink', import.meta.url), 'utf8');
...
const mapping = app.flora.completeHero.mapping(HERO_ID), target = app.flora.adapter.hero().regions.find(region => region.kind === 'leaf-region').regionId;
...
const manual = addManualStroke(app, 'manual-r1c-retention');
...
// Restore exact PREVIEW_ONLY document before roundtrip validation.
app.flora.reloadDocument(serialized);
const savedAgain = app.flora.serializeDocument();
const reload = createWP6App(); reload.flora.reloadDocument(savedAgain);
...
rollbackApp.flora.adapter.execute = (action, options) => { if (action.type === 'paintRegion' && ++count === 17) throw new Error('WP9A-R1c injected rollback'); return original(action, options); };
...
const output = {
  schema: 'INK_FLORA_WP9A_R1C_RELIABILITY_V1',
  initial,
  localRecompile: { result: { ok: local.ok, atomic: local.atomic, heroId: local.heroId, recipeIds: local.recipeIds, createdStrokeCount: local.createdStrokeIds?.length || 0, removedStrokeCount: local.removedStrokeIds?.length || 0, historyEntriesAdded: local.historyEntriesAdded }, target, nonTargetBefore, nonTargetAfter, nonTargetUnchanged: nonTargetBefore === nonTargetAfter, manualPreserved },
