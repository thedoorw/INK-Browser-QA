...
| Dead Core import bindings | `boundsContains`, `boundsIntersect`, `polygonContains`, `allObjects`, `renderTiledCanvas` appeared only in import declarations | removed unused bindings | no live call site; reduce stale API wiring without removing modules | defining modules and exports unchanged | low | exact-symbol scan, JS syntax scan, graph remeasurement; Windows M4 gate pending |
| Dead Studio import binding | `createImageSnapshot` appeared only in the import declaration | removed unused binding | no live call site; does not reduce Raster/Image capability | `src/image/image-core.js` export unchanged | low | exact-symbol scan, JS syntax scan; Windows M4 gate pending |
