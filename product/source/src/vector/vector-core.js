...
export function drawVectorObject(ctx,object,{drawObject}={}){
  if(object.type==='path'){
    ...
    if(fill&&fill!=='none'){ctx.fillStyle=fill;ctx.fill(object.fillRule||'evenodd');}const expressiveDrawn=object.expressiveStroke?drawExpressivePathStroke(ctx,object):false;if(!expressiveDrawn&&object.stroke&&object.stroke!=='none'&&(object.strokeWidth||0)>0){tracePath(ctx,object);ctx.strokeStyle=object.stroke;ctx.lineWidth=object.strokeWidth;ctx.lineJoin=object.lineJoin||'round';ctx.lineCap=object.lineCap||'round';ctx.miterLimit=object.miterLimit||4;ctx.setLineDash?.(object.dash||[]);ctx.lineDashOffset=object.dashOffset||0;ctx.stroke();}return true;
  }
...
export function vectorObjectToSVG(object,defs=[]){
  if(object.type==='path'){let fill=object.fill||'none';if(object.gradient?.stops?.length){const gid=object.gradient.id||`grad_${object.id.replace(/[^a-z0-9_-]/gi,'_')}`,radial=object.gradient.type==='radial';defs.push(`<${radial?'radialGradient':'linearGradient'} id="${gid}" ${radial?`cx="${object.gradient.x2??.5}" cy="${object.gradient.y2??.5}" r="${object.gradient.r??.5}"`:`x1="${object.gradient.x1??0}" y1="${object.gradient.y1??0}" x2="${object.gradient.x2??1}" y2="${object.gradient.y2??0}"`} gradientUnits="${object.gradient.units||'objectBoundingBox'}"${object.gradient.matrix?` gradientTransform="${matrixSVG(object.gradient.matrix)}"`:''}>${object.gradient.stops.map(s=>`<stop offset="${s.offset}" stop-color="${esc(s.color)}"${s.opacity==null?'':` stop-opacity="${s.opacity}"`}/>`).join('')}</${radial?'radialGradient':'linearGradient'}>`);fill=`url(#${gid})`;}
    ...
    const expressive=normalizeExpressiveStroke(object.expressiveStroke,{color:object.stroke,width:object.strokeWidth}),svgStroke=expressive?.fallback?.color||object.stroke||'none',svgWidth=expressive?.fallback?.width??object.strokeWidth??0,expressiveAttrs=expressive?` data-ink-expressive-stroke="${expressive.format}@${expressive.version}" data-ink-expressive-fallback="ordinary-vector" data-ink-profile-samples="${expressive.profile.samples.length}"`:'';
    const blend=object.blendMode&&object.blendMode!=='source-over'&&object.blendMode!=='normal'?` style="mix-blend-mode:${esc(object.blendMode)}"`:``;return `<path id="${esc(object.id)}" data-ink-id="${esc(object.id)}" data-ink-type="path" data-ink-topology="${(object.subpaths||[]).map(s=>s.role).join(',')}"${expressiveAttrs} transform="${matrixSVG(object.matrix)}" opacity="${object.opacity??1}"${blend} fill="${esc(fill)}" fill-rule="${object.fillRule||'evenodd'}" stroke="${esc(svgStroke)}" stroke-width="${svgWidth}" stroke-linecap="${object.lineCap||'round'}" stroke-linejoin="${object.lineJoin||'round'}" stroke-miterlimit="${object.miterLimit||4}"${!expressive&&object.dash?.length?` stroke-dasharray="${object.dash.join(' ')}"`:''}${!expressive&&object.dashOffset?` stroke-dashoffset="${object.dashOffset}"`:''}${clip} d="${pathData(object)}"/>`;
  }
