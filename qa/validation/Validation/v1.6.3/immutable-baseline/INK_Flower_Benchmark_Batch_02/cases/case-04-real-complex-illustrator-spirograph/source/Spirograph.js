...
function main() {
    ...
    for (var i = 0; i < circles; i++) {
        ...
        var circle1 = createEllipse(aw, position.x, position.y * -1, radius * 2, radius * 2, color);
        circle1.zOrder(ZOrderMethod.SENDTOBACK);
        var circle2 = createEllipse(aw, position.x, position.y * -1, radius * 2 - diameter, radius * 2 - diameter, color);
        circle2.zOrder(ZOrderMethod.SENDTOBACK);

        var circle3 = createEllipse(aw, position.x, position.y * -1, radius * 2 + diameter, radius * 2 + diameter, color);
        circle3.zOrder(ZOrderMethod.SENDTOBACK);

        position.x += radius * Math.cos(theta);
        position.y += radius * Math.sin(theta);
        var circle4 = createEllipse(aw, position.x, position.y * -1, diameter, diameter, setRGBColor(255));
        circle4.filled = true;
        circle4.fillColor = setRGBColor(200);
