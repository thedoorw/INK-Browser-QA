...
const OUT = path.join(ROOT, 'Runtime_Evidence', 'WP7');
fs.mkdirSync(OUT, { recursive: true });
const heroId = 'wp7-refined-hero';
const planId = 'wp7-refined-plan';
const makePlan = (seed = 606) => {
  const plan = createWP6Plan(seed, { heroId, planId, petalCount: 10 });
  plan.metadata = { source: 'ai', label: 'WP7 Refined A4 Hero Flower Benchmark C3' };
  plan.crownPlan.metadata = { source: 'ai', label: 'WP7 Refined Single Crown' };
  plan.stemPlan.metadata = { source: 'ai', label: 'WP7 Refined Stem' };
  plan.leafPlans[0].metadata = { source: 'ai', label: 'WP7 Refined Left Leaf', side: 'left' };
  plan.leafPlans[1].metadata = { source: 'ai', label: 'WP7 Refined Right Leaf', side: 'right' };
  plan.backgroundPlan.metadata = { source: 'ai', label: 'WP7 Refined Quiet Background' };
  return plan;
};
...
fs.writeFileSync(path.join(OUT, 'WP7_Before_Local_Edit.ink.json'), documentBeforeLocal);
fs.writeFileSync(path.join(OUT, 'WP7_After_Local_Edit.ink.json'), serialized);
...
rollbackApp.flora.adapter.execute = (action, options) => {
  if (action.type === 'paintRegion' && ++paints === 23) throw new Error('WP7 runtime injected pass failure');
...
oldLikeRecipe.metadata.label = 'WP6-compatible Recipe without WP7 refinement controls';
const recipeDiff = { legacyCompatible: oldLikeRecipe, wp7Refined: refinedRecipe, addedParameters: Object.keys(refinedRecipe.refinement || {}) };
fs.writeFileSync(path.join(OUT, 'Visual_Refinement_Recipe_Diff.json'), JSON.stringify(recipeDiff, null, 2) + '\n');
fs.writeFileSync(path.join(OUT, 'A4_Hero_Refined_Plan.json'), JSON.stringify(plan, null, 2) + '\n');
fs.writeFileSync(path.join(OUT, 'A4_Hero_Refined_Compiled_Action_Log.json'), JSON.stringify({ recipes: compiled.recipes, preview: compiled.preview, checks: compiled.checks }, null, 2) + '\n');
const evidence = {
  schema: 'INK_FLORA_WP7_TRANSACTION_EVIDENCE_V1', imageModelUsed: false, speciesUsed: false,
