...
PLAN = json.loads((OUT / "A4_Hero_Refined_Plan.json").read_text(encoding="utf-8"))
...
with sync_playwright() as p:
    ...
    refined = OUT / "01_wp7_refined_complete.png"
    refined.write_bytes(payload)
    (OUT / "Benchmark_C3_Refined_A4_Hero.png").write_bytes(payload)
    browser.close()

errors = [item for item in messages if item["type"] in {"error", "pageerror"}]
runtime = {
    "schema": "INK_FLORA_WP7_BROWSER_RUNTIME_V1",
