...
OUT=ROOT/'Runtime_Evidence'/'NonPeriodicFill'; OUT.mkdir(parents=True,exist_ok=True)
NEW=ROOT/'FLR012_R1c1_NonPeriodic_Fill_PREVIEW_ONLY.png'
SMALL=ROOT/'FLR012_R1c1_NonPeriodic_Fill_Small_View_PREVIEW_ONLY.png'
INK=ROOT/'FLR012_R1c1_NonPeriodic_Fill_PREVIEW_ONLY.ink'
OLD=ROOT/'FLR012_PREVIEW_ONLY_Painted_Retention.png'
OLD_INK=ROOT/'FLR012_PREVIEW_ONLY_Painted_Retention.ink'
...
with sync_playwright() as p:
 ...
 for name,ids in groups.items():
  ex=page.evaluate(filt,{'ids':ids});data=decode(ex['data']);(OUT/f'R1c1_Isolated_{name}.png').write_bytes(data);new_iso[name]=(data,ex['width'],ex['height'])
 ...
 old_iso={}
 for name,ids in groups.items():
  ex=page.evaluate(filt,{'ids':ids});data=decode(ex['data']);(OUT/f'R1c_Isolated_{name}.png').write_bytes(data);old_iso[name]=(data,ex['width'],ex['height'])
...
side=Image.new('RGB',(w*2+40,h+55),'white');d=ImageDraw.Draw(side);side.paste(old,(5,45));side.paste(im,(w+35,45));d.text((150,12),'R1c PREVIEW',fill='black');d.text((w+150,12),'R1c.1 PREVIEW_ONLY',fill='black');side.save(OUT/'R1c_vs_R1c1_Full.png')
smallside=Image.new('RGB',(500,340),'white');sd=ImageDraw.Draw(smallside);smallside.paste(fit(old,(212,300)),(15,30));smallside.paste(fit(im,(212,300)),(270,30));sd.text((55,8),'R1c',fill='black');sd.text((320,8),'R1c.1',fill='black');smallside.save(OUT/'R1c_vs_R1c1_Small.png')

metrics={}; retention={}
for name,ids in groups.items():
 regs=[r for r in regions if r['regionId'] in ids];gm=path_mask(regs,(w,h));na=alpha(new_iso[name][0]);oa=alpha(old_iso[name][0])
 metrics[name]={'R1c':spatial_metrics(old_iso[name][0],gm),'R1c1':spatial_metrics(new_iso[name][0],gm)}
...
spatial={'schema':'INK_FLORA_WP9A_R1C1_SPATIAL_METRICS_V1','thresholds':thresholds,'metrics':metrics,'checks':checks,'allChecksPass':all(all(x.values()) for x in checks.values())}
...
sheet.save(OUT/'WP9A_R1c1_NonPeriodicFill_Evidence_Contact_Sheet.png')
