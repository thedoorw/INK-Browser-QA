...
STATUS: `ACTIVE / AUTHORITATIVE_UI_GOVERNANCE / UR_OWNED`

BASELINE_MAIN: `79b76ccafd6e1d3443d2565fcc6d4f284e1d6eac`

AUTHORITY:
- UR owns enforcement, review, amendment and UI-health closure while changes remain UI-only.
- MR is escalation authority only when compliance would require a frozen Core / cross-lane / product-global change.
...
Escalate to MR only if the exception requires changing a non-UI authority.
...
If one bounded work order increases `styles.css` by more than 5% without deleting/replacing obsolete presentation, UR must explicitly decide whether:
...
MR authorization is required only if the proposed change crosses the UI lane or alters a product-global/Core contract.
...
Normal UI-health result:

`STOP → UR_REVIEW → UI_REVISE / UI_HOLD`

Only this class escalates to MR:

`FROZEN_CORE_MUTATION > 0 → STOP → MR / INTEGRATION_REQUIRED`

UR may not transfer ordinary UI-health failures to MR merely because the implementation has already reached an integration or main stage.

## 14. Periodic health review

UR must run health review at minimum:
- after each Photoshop alignment stage;
- before UI promotion to main;
- after promotion on the integrated exact main SHA;
- after any UI work order with broad CSS changes.

The health report must compare against the post-cleanup baseline, not only against the immediately previous commit.

A branch PASS is provisional. Final UI health closure requires the integrated main result to pass.

This prevents gradual regression across many individually small changes and prevents responsibility from being deferred until after integration.
